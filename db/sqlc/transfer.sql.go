...
const getTransferByAccounts = `-- name: GetTransferByAccounts :one
SELECT id, from_account_id, to_account_id, amount, created_at 
FROM transfers
...
type GetTransferByAccountsParams struct {
	FromAccountID int64 `json:"from_account_id"`
	ToAccountID   int64 `json:"to_account_id"`
}

func (q *Queries) GetTransferByAccounts(ctx context.Context, arg GetTransferByAccountsParams) (Transfer, error) {
	row := q.db.QueryRowContext(ctx, getTransferByAccounts, arg.FromAccountID, arg.ToAccountID)
	var i Transfer
	err := row.Scan(
		&i.ID,
		&i.FromAccountID,
...
const listTransferByAccounts = `-- name: ListTransferByAccounts :many
SELECT id, from_account_id, to_account_id, amount, created_at 
FROM transfers
WHERE from_account_id = $1
AND to_account_id = $2
...
type ListTransferByAccountsParams struct {
	FromAccountID int64 `json:"from_account_id"`
	ToAccountID   int64 `json:"to_account_id"`
	Limit         int32 `json:"limit"`
	Offset        int32 `json:"offset"`
...
func (q *Queries) ListTransferByAccounts(ctx context.Context, arg ListTransferByAccountsParams) ([]Transfer, error) {
	rows, err := q.db.QueryContext(ctx, listTransferByAccounts,
		arg.FromAccountID,
		arg.ToAccountID,
		arg.Limit,
		arg.Offset,
